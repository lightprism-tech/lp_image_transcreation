...
│   │   └── samples/            # Sample images (e.g. testing_image.jpg)
│   ├── knowledge_base/         # Cultural knowledge graph (countries_graph.json, .pkl, .jsonl)
│   └── output/
│       ├── debug/              # Visualizations (BBox overlays)
│       └── json/               # JSON when saved via --output
├── scripts/
...
**Note:** The pipeline does **not** save JSON to disk by default to maintain statelessness. JSON is returned to the caller (API/CLI). CLI users can save outputs using the `--output` flag (e.g. to `data/output/json/`).
...
```
data/knowledge_base/           # Generated outputs (do not edit by hand)
├── countries_graph.json      # Graph in JSON (nodes + edges)
├── countries_graph.pkl       # Same graph as pickle
└── countries_graph.jsonl     # Line-delimited nodes/edges

scripts/knowledge_graph/       # Source and generator
├── countries.json            # Country/culture data (edit here)
└── generator.py              # Regenerate: python scripts/knowledge_graph/generator.py
```

Planned extended structure (cultures, avoid lists, style priors):
